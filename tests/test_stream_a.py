    ...
    def test_viewer_auth_origin_and_html_inert(self):
        with tempfile.TemporaryDirectory() as scratch:
            ...
            try:
                ...
                self.assertIn("viewport-fit=cover", viewer_mod.APP)
                self.assertIn('rel="icon"', viewer_mod.APP)
                self.assertIn('rel="apple-touch-icon"', viewer_mod.APP)
                self.assertIn('name="theme-color"', viewer_mod.APP)
                self.assertIn("id=\"filters\"", viewer_mod.APP)
                self.assertIn("id=\"filters\" open", viewer_mod.APP)
                self.assertIn("Back to recordings", viewer_mod.JS)
                ...
                self.assertEqual(favicon_headers.get("Content-Type"), "image/png")
